    ...
    private void init(Context context) {
        if (!isInEditMode()) {
            if(Session.get_user_language(context).equals("en")) {
                Typeface tf = Typeface.createFromAsset(getContext().getAssets(), "fonts/GothamRounded-Medium.otf");
                setTypeface(tf);
            }else{
                Typeface tf = Typeface.createFromAsset(getContext().getAssets(), "fonts/HacenTunisiaBd.ttf");
                setTypeface(tf);
            }
        }
